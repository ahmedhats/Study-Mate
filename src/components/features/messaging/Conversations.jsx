...
const getConversationTitle = (conversation, currentUserId) => {
  ...
  return (
    conversation.title || conversation.communityName || "Untitled Conversation"
  );
};
...
const getConversationAvatar = (conversation, currentUserId) => {
  ...
  if (conversation.type === "DM") {
    ...
    return otherParticipant?.userId?.profileImage || undefined;
  }
...
const ConversationList = ({
  ...
  const hasUnreadMessages = (conversation) => {
    if (!currentUser || !conversation || !conversation.participants)
      return false;
    ...
    const lastReadTime = userParticipant.lastReadTimestamp
      ? new Date(userParticipant.lastReadTimestamp).getTime()
      : 0;
  ...
  const renderConversationItem = (conversation) => {
    ...
    if (conversation.type === "DM") {
      ...
      avatarComponent = (
        <Avatar
          src={getProfileImageUrl(avatarSrc) || undefined}
          style={{
            backgroundColor: !avatarSrc ? avatarColor : undefined,
            marginRight: 12,
          }}
    ...
    return (
      ...
      >
        <List.Item.Meta
          avatar={
            isUnread ? <Badge dot>{avatarComponent}</Badge> : avatarComponent
          }
          title={
            <Text strong={isUnread} style={{ marginBottom: 0 }}>
              {title}
            </Text>
          }
          description={
            conversation.lastMessage && conversation.lastMessage.text ? (
              ...
              >
                ...
                conversation.lastMessage.senderId !== currentUserId
                  ? `${conversation.lastMessage.senderName || "User"}: `
                  : ""}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <List
        ...
        style={{
          flex: "1 1 auto",
          overflowY: "auto" /* any other list specific styles */,
        }}
      />
      <div
        style={{
