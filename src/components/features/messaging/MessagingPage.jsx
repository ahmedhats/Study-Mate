...
import { MessageOutlined, MenuOutlined, ReloadOutlined } from '@ant-design/icons';
...
const MessagingPage = () => {
  ...
  const [conversationNotFoundWarning, setConversationNotFoundWarning] = useState(false);
  ...
  useEffect(() => {
    if (routeConversationId && routeConversationId !== "undefined") {
      ...
      if (currentConversation && currentConversation._id === routeConversationId && currentUser?.user?._id) {
      ...
      const conversationExists = conversations.some(c => c._id === activeConversationId);
      if (conversationExists || loading.initialConversations) {
        console.log(`MessagingPage: Redirecting to stored/active conversation: ${activeConversationId}`);
        // Ensure navigation doesn't happen if already on the correct path or during initial load if activeID is already the target
        if (`/messages/${activeConversationId}` !== window.location.pathname.replace(window.location.hash, '')) {
            navigate(`/messages/${activeConversationId}`);
        }
      } else if (!loading.initialConversations) {
        console.log('MessagingPage: Stored conversation not found after loading, clearing.');
  ...
  }, [routeConversationId, actions, activeConversationId, isMobile, currentConversation, currentUser, navigate, conversations, loading.initialConversations]);

  // Effect to handle conversation not found and retry logic
  useEffect(() => {
    if (!routeConversationId || routeConversationId === "undefined" || loading.initialConversations) {
      // If no route ID, or still loading initial conversations, reset retry count and do nothing further here.
      if(retryCount > 0) setRetryCount(0);
      return;
    }

    const conversationExists = conversations.some(c => c._id === routeConversationId);

    if (!conversationExists) {
      console.log(`MessagingPage: Conversation ${routeConversationId} not found in list. Retry: ${retryCount}`);
      if (retryCount < 2) {
        setRetryCount(prev => prev + 1);
        handleForceRefresh();
      } else {
        console.log(`MessagingPage: Conversation ${routeConversationId} not found after ${retryCount} attempts. Clearing and redirecting.`);
  ...
  }, [routeConversationId, conversations, loading.initialConversations, retryCount, actions, handleForceRefresh, navigate]);
  ...
  const getConversationTitle = () => {
    ...
    if (!currentUser || !currentUser.user || !currentUserId) {
        console.warn("getConversationTitle: currentUser, currentUser.user or currentUserId is not available yet. CurrentUser:", currentUser);
        return currentConversation.type === "DM" ? "Direct Message" : (currentConversation.communityName || "Group Chat");
    }
    
    console.log("getConversationTitle: currentConversation:", currentConversation);
    ...
    if (currentConversation.type === "DM") {
      if (!currentConversation.participants || currentConversation.participants.length === 0) {
        console.warn("DM conversation has no participants:", currentConversation.participants);
      ...
      console.log("getConversationTitle: DM otherParticipant:", otherParticipant);
  ...
  const canSendMessages = () => {
    ...
    if (!currentConversation.participants) {
      console.error("Conversation is missing participants array:", currentConversation);
      return false;
    }
    ...
    if (currentConversation.type === "DM") {
      const isParticipant = currentConversation.participants.some(
        p => {
          ...
          const currentUserId = currentUser?.user?._id || currentUser?.user?.id || currentUser?._id || currentUser?.id;
          console.log("Comparing participant:", participantId, "with current user ID:", currentUserId);
  ...
  const conversationListElement = (
    <Card
      title={
        <div className="conversation-list-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>Messages</span>
          <Button 
            type="text" 
      ...
      styles={{
        header: { flexShrink: 0 },
        body: { padding: 0, flex: '1 1 auto', display: 'flex', flexDirection: 'column', overflow: 'hidden' }
      }}
    >
      ...
      {error && error.type === 'fetch' && error.message.includes('conversations') && 
        <Alert 
          message="Error" 
          ...
          style={{ margin: '10px' }}
          action={
            <Button size="small" onClick={handleForceRefresh} disabled={refreshingData}>
              Retry
            </Button>
          }
        />
      ...
      {!loading.initialConversations && !refreshingData && !conversations.length && !error && (
        <Empty description="No conversations yet." style={{ padding: '20px', margin: 'auto' }} />
      )}
      {!loading.initialConversations && !refreshingData && conversations.length > 0 && (
          <ConversationList 
    ...
    <Row gutter={16} style={{ height: 'calc(100vh - 64px)' }}>
      ...
      >
        {(routeConversationId && currentConversation) ? (
          ...
          >
            {error && error.type === 'fetch' && error.message.includes('messages') && 
              <Alert 
                ...
                style={{ margin: '10px' }}
                action={
                  <Button size="small" onClick={() => actions.fetchMessagesForConversation(routeConversationId)}>
                    Retry
                  </Button>
                }
              />
          ...
          >
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <Spin size="large" />
              <div style={{ marginTop: '10px' }}>Loading conversation data...</div>
              {retryCount >= 2 && (
