...
import CommunityStudySessions from "./CommunityStudySessions";
import {
  getCommunityDetails,
...
const CommunityDetail = ({ communityId }) => {
  ...
  const { conversations: contextConversations, actions: messagingActions } =
    useMessaging();
  ...
  useEffect(() => {
    if (
      ...
      const foundConv = contextConversations.find(
        (conv) =>
          conv.type === "COMMUNITY" && conv.communityId === community._id
      );
      if (foundConv) {
        ...
        console.warn(
          `Conversation for community ${community._id} not found in context.`
        );
          ...
          >
            {communityConversationId ? (
              ...
              <div style={{ textAlign: "center", padding: "50px" }}>
                {activeTab === "chat" && (!community || loading) ? (
                  <Spin tip="Loading community details..." />
                ) : (
                  <Empty
                    description={
